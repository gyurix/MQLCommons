...
//|                                                                  |
//+------------------------------------------------------------------+
#property copyright "gyurix"
#property link      "https://github.com/gyurix/"
#property description "MQL4 Programming Basic Library"
#property strict
...
void reportError(string msg){
   ...
   Alert("Error #",code," at ",msg,": ",getErrorMessage(code));
}
...
string getUninitReasonText(int reasonCode) { 
   switch(reasonCode){ 
      ...
      case REASON_PARAMETERS: 
         return "Input-parameter was changed";
      case REASON_RECOMPILE: 
         return "Program "+__FILE__+" was recompiled";
      case REASON_REMOVE: 
         return "Program "+__FILE__+" was removed from chart";
      case REASON_TEMPLATE: 
         return "New template was applied to chart";
   } 
...
string getPeriodName(int period){
   if (period < 60)
      return string(period)+" "+(period==1?"Minute":"Minutes");
   if (period < 24*60)
      return string(period/60)+" "+(period==60?"Hour":"Hours");
   if (period < 7*24*60)
      return string(period/60/24)+" "+(period==24*60?"Day":"Days");
   return string(period/60/24/7)+" "+(period==7*24*60?"Week":"Weeks");
}
...
string formatTime(long time){
   return TimeToStr(OrderOpenTime(),TIME_DATE|TIME_SECONDS);
}

double usedMarginPct(){
   return AccountMargin() * 100 / AccountEquity();
}

double freeMarginPct(){
   return AccountFreeMargin() * 100 / AccountEquity();
}

double marginLevel(){
   return AccountEquity() * 100 / (AccountMargin()+0.01);
}